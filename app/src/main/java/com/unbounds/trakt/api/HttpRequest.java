...
public class HttpRequest {
	...
	public HttpResponse execute() throws HttpException {
		...
		try {
			...
			throw new HttpException.HttpIOException(exception, request.method(), request.url().toString());
		}
	}
	...
	private String parseResponse(final Request request, final Response response) throws HttpException {
		...
		try {
			...
			if (statusCode != 200 && statusCode != 201 && statusCode != 202 && statusCode != 204) {
				throw new HttpException(statusCode, body, request.method(), request.url().toString());
			}
		}
